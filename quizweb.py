...
# Logic Chấm điểm (Kết hợp từ code cũ của cậu)
if st.button("Kiểm tra đáp án") and not st.session_state.answered:
    ...
    st.rerun() # Rerun để cập nhật trạng thái đã trả lời
...
if st.session_state.answered:
    if st.button("Câu tiếp theo ➡️"):
        if st.session_state.current_index < end_range - 1:
            ...
            st.balloons()
            st.success(f"🎉 Hoàn thành chuyên đề! Điểm của bạn: {st.session_state.score}")
